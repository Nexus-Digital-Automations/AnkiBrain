...
class ChatAIModuleAdapter:
    ...
    def __init__(self):
        self.scriptManager = ExternalScriptManager(
            python_path=python_path, script_path=ChatAI_module_init_path
        )
    ...
    async def call(self, cmd: IC, **kwargs) -> CallResponse:
        ...
        print(
            f"<ChatAIModuleAdapter> Received output from ChatAI module: {json.dumps(out)}"
        )
    ...
    async def ask_conversation_with_documents(
        self, query: str
    ) -> AskWithDocumentsResponse:
    ...
    async def ask_conversation_no_documents(
        self, query: str
    ) -> AskWithoutDocumentsResponse:
    ...
    async def explain_topic(self, topic, options):
        output = await self.call(IC.EXPLAIN_TOPIC, topic=topic, options=options)
    ...
    async def generate_cards(
        self, text: str, custom_prompt: str, card_type: CardType, language: str
    ) -> dict:
