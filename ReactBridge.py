...
class ReactBridge:
    ...
    def __init__(self, app: AnkiBrain):
        ...
        self.app.sidePanel.webview.page().react_data_received.connect(
            self.handle_react_data_received
        )
    ...
    def handle_react_data_received(self, data: str):
        ...
        asyncio.run_coroutine_threadsafe(
            self.a_handle_react_data_received(json.loads(data)), loop
        )
    ...
    async def a_handle_react_data_received(self, data: dict):
        try:
            ...
            if cmd == IC.EXPLAIN_TOPIC:
                ...
                self.send_cmd(IC.DID_EXPLAIN_TOPIC, output, commandId)
                ...
                try:
                    output = await self.app.chatAI.generate_cards(
                        text=text,
                        custom_prompt=custom_prompt,
                        card_type=card_type,
                        language=language,
                    )
                    self.send_cmd(IC.DID_GENERATE_CARDS, output, commandId)
                except Exception as e:
                    # self.send_cmd(IC.FAILED_GENERATE_CARDS, {'error': json.loads(str(e))})
                    self.send_cmd(
                        IC.DID_GENERATE_CARDS, error=str(e), commandId=commandId
                    )
                ...
                output = await self.app.chatAI.ask_conversation_with_documents(
                    data["query"]
                )
                self.send_cmd(IC.DID_ASK_CONVERSATION_DOCUMENTS, output, commandId)

            elif cmd == IC.ASK_CONVERSATION_NO_DOCUMENTS:
                output = await self.app.chatAI.ask_conversation_no_documents(
                    data["query"]
                )
                self.send_cmd(IC.DID_ASK_CONVERSATION_NO_DOCUMENTS, output, commandId)

            elif cmd == IC.CLEAR_CONVERSATION:
                await self.app.chatAI.clear_conversation()
                print(
                    "<ReactBridge> cleared conversation, now sending confirmation to react"
                )
                ...
                try:
                    ...
                    self.send_cmd(
                        IC.DID_ADD_DOCUMENTS, error=str(e), commandId=commandId
                    )
                ...
                try:
                    ...
                    self.send_cmd(
                        IC.DID_UPLOAD_DOCUMENT, error=str(e), commandId=commandId
                    )
                ...
                try:
                    ...
                    self.send_cmd(
                        IC.DID_SPLIT_DOCUMENT, error=str(e), commandId=commandId
                    )
                ...
                try:
                    res = await fetch(
                        url, verb, data
                    )  # todo try/except, send err to js
                    self.send_cmd(IC.DID_NETWORK_REQUEST, data=res, commandId=commandId)
                except Exception as e:
                    self.send_cmd(
                        IC.DID_NETWORK_REQUEST, error=str(e), commandId=commandId
                    )
